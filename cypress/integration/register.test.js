...
describe('Register', () => {
    ...
    it('should validate the password field', () => {
        cy
            ...
            .get('input[name=password]').type('greaterthan10')
            .get('validation-list')
            .get('validation-list > .error').contains(
        ...
        cy.get('.navbar-burger').click()
        cy.get('.navbar-item').contain('Log In').click()
        cy.get('.navbar-item').contains('Register').click()
        cy.get('.validation-list > .error').contains(
            'Password must be greater than 10 characters.')
    });
    it('shold throw an error is username is taken', () => {
        cy
            .visit('/register')
            .get('input[name="username"]').type(username)
            .get('input[name="email"]').type(email)
    ...
    it('shold throw an error if email is taken', () => {
        cy
            .visit('/register')
